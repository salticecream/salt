...
void salt::fill_types(int word_size) {
	salt::all_types = std::unordered_map<std::string, const salt::Type*>({
		{"unknown", nullptr},
		{"__SaltErrorTy",	new salt::Type("__SaltErrorTy",		llvm::Type::getVoidTy(*global_context),		0)},
		{"__SaltNeverTy",	new salt::Type("__SaltNeverTy",		llvm::Type::getVoidTy(*global_context),		0)},
		...
		{"__SaltPtr",		new salt::Type("__SaltPtr",			llvm::PointerType::get(*global_context, 0),	512000,	false)},
...
std::string TypeInstance::str() const {
	if (!type)
		return "ERROR TYPE [nullptr]";

	if (type != SALT_TYPE_PTR)
		return type->name;
	...
	std::string pointee_to_print = type ? type->name : "<null>";
